...
def prediction_probabilities_plot(df, logger, test_indices, y_pred_proba) -> None:
    ...
    ax.fill_between(test_indices, 0.5, y_pred_proba, where=y_pred_proba >= 0.5, alpha=0.3, color='#27ae60', label='Predicted Up')
    ax.fill_between(test_indices, y_pred_proba, 0.5, where=y_pred_proba < 0.5, alpha=0.3, color='#e74c3c', label='Predicted Down')
...
def visualization(acc, clf, df, features, split_idx, test_df, y_pred, y_test) -> None:
    ...
    axes[0, 0].plot(df.index[:split_idx], df['value'][:split_idx], color='#2c3e50', linewidth=1, label='Train')
    axes[0, 0].plot(df.index[split_idx:], df['value'][split_idx:], color='#e74c3c', linewidth=1, label='Test')
    ...
    incorrect = ~correct
    axes[0, 1].scatter(test_indices[correct], test_df['value'][correct], c='#27ae60', s=30, alpha=0.6, label='Correct')
    axes[0, 1].scatter(test_indices[incorrect], test_df['value'][incorrect], c='#e74c3c', s=30, alpha=0.6, label='Incorrect')
    axes[0, 1].set_title(f'Classification Performance (Accuracy: {acc:.3f})', fontsize=12)
    axes[0, 1].legend()
    axes[0, 1].set_ylabel('Value')
    axes[0, 1].set_xlabel('Index')
    cm = confusion_matrix(y_test, y_pred)
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', ax=axes[1, 0], xticklabels=['Down', 'Up'], yticklabels=['Down', 'Up'], cbar_kws={'label': 'Count'})
    axes[1, 0].set_title('Confusion Matrix', fontsize=12)
    axes[1, 0].set_ylabel('Actual')
    axes[1, 0].set_xlabel('Predicted')
    importances = clf.feature_importances_
    feature_imp_df = pd.DataFrame({'feature': features, 'importance': importances}).sort_values('importance', ascending=True)
    axes[1, 1].barh(feature_imp_df['feature'], feature_imp_df['importance'], color='#3498db', alpha=0.7)
...
def seed() -> None:
    ...
    df = pd.DataFrame({'value': data, 'lag_1': data.shift(1), 'lag_2': data.shift(2), 'lag_3': data.shift(3), 'rate_of_change': data.diff(), 'acceleration': data.diff().diff(), 'rolling_mean_5': data.shift(1).rolling(window=5).mean(), 'rolling_std_5': data.shift(1).rolling(window=5).std()})
    df['target'] = (data.shift(-1) > data).astype(int)
    df = df.dropna()
    split_idx = int(len(df) * 0.8)
    train_df = df.iloc[:split_idx]
    test_df = df.iloc[split_idx:]
    features = ['lag_1', 'lag_2', 'lag_3', 'rate_of_change', 'acceleration', 'rolling_mean_5', 'rolling_std_5']
